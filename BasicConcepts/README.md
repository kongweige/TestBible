...
![alt text](./images/image-selenuim.png)

**显示等待和隐式等待区别**

|类型|	使用方式|	原理|	适用场景|
|------|------|------|------|
|直接等待|	time.sleep(等待时间)|强制线程等待|调试代码，时性添加|
|隐式等待|	driver.implicitly_wait(等待时间) |在时间范围内，轮询查找元素|	解决找不到元素问题，无法解决交互问题，也无法自定义等待条件|
|显式等待|	WebDriverWait(driver实例, 最长等待时间).until(条件函数)|设定特定的等待条件，轮询操作|	解决特定条件下的等待问题，比如点击等交互性行为，同时可以自定义等待条件|

**八大定位**

id、name、tag、link、css选择器，xpath

**如何判断一个界面上元素是否存在**

通过查看当前界面的dom，搜该元素是否存在，如果是脚本自动化运行过程中没找到，通过打印page_source，即可了解到该元素在运行过程中是否存在。

**元素定位到了，但是点击无效（没有报错）如何解决？**
* 原因：异步加载js导致点击不到
* 解决：循环点击该按钮。直到生效为止（显示等待）
* 原因：app突然出现弹窗，导致元素遮挡
* 解决：通过添加黑名单异常处理解决

**selenium中隐藏元素如何定位？**
* 原因：隐藏元素可以直接定位，但是无法直接点击或交互
* 解决：使用js执行交互操作
...
**元素定位不到**
* 定位不确定： 在console先测试定位是否正确
* 界面还没加载完成：先添加强制等待验证，在是否隐式等待或显示等待优化
* 动态ID：使用css或xpath的相对定位
* 界面有iframe：切换到ifame后再定位(switch_to())
* 界面切换window: 切换到对应的窗口后定位
